...
import {
  ...
  function Amin({telePiano, handleNewScale, handleRestart, notePlayed, setNotePlayed, scale, setScale, allScales, pianoType, setPianoType}) {

    function playAminNote(event) {
      ...
      let arrNotesAmin =  allScales && allScales[12] ? allScales[12]["notes"].split(', ').map((note)=>  parseInt(note)) : null

      function compareAminScales(arrNotesAmin, scale){
        if (scale.length === 8){ 
          return arrNotesAmin.every((note, index) => {
              return note === scale[index]
          })
        }
    }
console.log(arrNotesAmin)
console.log(scale)

// !!!!!!!!!!! Change pianotype to 5 when create the G major scale
    return (
        <div>
          {compareAminScales(arrNotesAmin, scale) && pianoType === 5  ? <><h2>Good Job</h2><button  className="new_scale_button" onClick={handleNewScale}>Click for a new Scale</button></> : pianoType === 5 ? <h3>Play all 8 correct notes in the A minor scale!</h3>: null}
        {scale.length === 8 & !compareAminScales(arrNotesAmin, scale) && pianoType === 5  ? <><h3>Good Try! Don't Worry!</h3><button className="new_scale_button" onClick={handleRestart}>Try Again</button></>: null}

          {pianoType === 5 ?  <div className="piano" onKeyDown={playAminNote} tabIndex={1}>
