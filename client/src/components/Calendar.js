  ...
  return user && user["private_classes"] ? (
    <>
      ...
      <div className="calendar_grid_container">
        ...
        {user.admin !== "1"
          ? user["private_classes"].map((privateClass) => (
              <CalendarCard privateClass={privateClass} key={privateClass.id} />
            ))
          : // If user is admin then render all upcoming classes
            privateClasses.map((oneClass) => (
              <CalendarAdminCard oneClass={oneClass} />
            ))}
      </div>
    </>
  ) : error && error["error"] ? (
