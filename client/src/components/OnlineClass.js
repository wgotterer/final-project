...
function OnlineClass({ user, error, setUser }) {
  ...
  function handleFilterSearch(value) {
    setClassToDisplay(
      allOnlineClasses.filter((filterclass) => {
  ...
  function filterByCategory(value) {
    if (value === "all") {
      ...
      setClassToDisplay(
        allOnlineClasses.filter((oneClass) => {
  ...
  return allOnlineClasses && user ? (
    <div>
      ...
      <button className="sign_up_button" onClick={handleShowPopUp}>
        Sign Up
      </button>
      <Popup
        user={user}
      ...
      <div className="online_grid_container">
        {classToDisplay
          ? classToDisplay.map((oneOnlineClass) => (
              <OnlineClassCard
