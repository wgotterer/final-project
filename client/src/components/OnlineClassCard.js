...
function OnlineClassCard({
  ...
  function handleSubmitPurchase(e) {
    e.preventDefault();
    if (
      user["online_classes"].some(
        (oneClass) => oneClass.id === oneOnlineClass.id
      )
    ) {
      ...
      fetch("/api/user_online_classes", {
        ...
        .then((newOnlineClass) => {
          ...
          setUser((userData) => ({
            ...userData,
            online_classes: [
              ...userData["online_classes"],
              newOnlineClass["online_class"],
            ],
          }));
  ...
  return oneOnlineClass &&
    user &&
    oneOnlineClass.available &&
    oneOnlineClass.available === true ? (
    <div className="online_class">
      ...
      {user.admin && user.admin === "1" ? (
        <>
          <button className="edit_button" onClick={handleEditClass}>
            {showEditForm ? "Close" : "Edit"}
          </button>

          {showEditForm ? (
            <EditFormClass
              setClassToDisplay={setClassToDisplay}
              classToDisplay={classToDisplay}
              setShowEditForm={setShowEditForm}
              oneOnlineClass={oneOnlineClass}
            />
          ) : null}
          <div className="delete_button_container">
            <button className="delete_button" onClick={handleDeleteClass}>
              Delete Class
            </button>
          </div>
        </>
      ...
      {showBuyForm ? (
        <div className="popup">
          <div className="popup-inner">
            ...
            <button className="buy_button" onClick={handleShowForm}>
              {" "}
              {showBuyForm ? "Close" : "BUY"}
            </button>
          </div>
        </div>
      ) : null}
