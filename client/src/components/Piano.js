...
function Piano({ user, error }) {
  ...
  return user ? (
    <div className="pianoPage">
      <h3>
        Practice your scales by playing the correct notes on your keyboard
      </h3>

      <StartScale
        handleMajScale={handleMajScale}
        handleMinScale={handleMinScale}
        handleNewScale={handleNewScale}
        pianoType={pianoType}
        setPianoType={setPianoType}
      />
      ...
      {majScale ? (
        <>
          ...
          {pianoType > -1 ? (
            <button className="new_scale_button" onClick={handleNewGame}>
              Quit
            </button>
          ) : null}
        </>
      ...
      {minScale ? (
        <>
          ...
          {/* use pianoType state to check state.  */}
          {/* make all scales into one component */}
          {/* JSON file that holds the scale information.. based on the index it will render */}
          <Cmin
            telePiano={telePiano}
            handleNewScale={handleNewScale}
            handleRestart={handleRestart}
